...
class RKF78VanDerPolTest : public testing::Test {
 protected:
  ...
  SingleThreadedIntegrator<BTRKF78, decltype(ode_van), AllocatedState<2>>
      integrator{ode_van};
};
...
TEST(RKF78ExpTest, IntegrationStepsAreConsistent) {
  ...
  EXPECT_DOUBLE_EQ(110131.78807367262, integrator.states.back()[n_var / 2]);
  EXPECT_DOUBLE_EQ(198237.21853261057, integrator.states.back()[n_var - 1]);
}
