...
int main() {
  auto ode = [](VectorState<2> const& x, VectorState<2>& dxdt) {
    dxdt = std::array{1.0, -x[1] * x[1] * x[1] + std::sin(x[0])};
  };
  ...
  integrator_RKF45.integrate(std::array{0.0, 0.0}, 0.0, 10.0,
                             std::array{tol, tol}, std::array{tol, tol});
  ...
  integrator_DOPRI5.integrate(std::array{0.0, 0.0}, 0.0, 10.0,
                              std::array{tol, tol}, std::array{tol, tol});
  ...
  start = std::chrono::high_resolution_clock::now();
  integrator_DVERK.integrate(std::array{0.0, 0.0}, 0.0, 10.0,
                             std::array{tol, tol}, std::array{tol, tol});
  end = std::chrono::high_resolution_clock::now();
  ...
  start = std::chrono::high_resolution_clock::now();
  integrator_RKF78.integrate(std::array{0.0, 0.0}, 0.0, 10.0,
                             std::array{tol, tol}, std::array{tol, tol});
  end = std::chrono::high_resolution_clock::now();
